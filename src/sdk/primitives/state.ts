 ...
 *
 * @example
 * import { Tokay, stateGet, stateSet } from 'tokay-sdk';
 *
 * // Read state
 * new Tokay()
...
function getStateDir(ctx) {
  return (
    ctx?.stateDir ||
    (ctx?.env?.TOKAY_STATE_DIR && String(ctx.env.TOKAY_STATE_DIR).trim()) ||
    path.join(os.homedir(), '.tokay', 'state')
  );
}
