...
function parseRunArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // Treat --dry-run as a Tokay flag only before positional command/pipeline
...
async function resolveWorkflowFile(candidate) {
  ...
  if (!['.tokay', '.yaml', '.yml', '.json'].includes(ext)) {
    throw new Error('Workflow file must end in .tokay, .yaml, .yml, or .json');
  }
...
function helpText() {
  return `tokay — HnsAi-native typed shell\n\n` +
    `Usage:\n` +
    `  tokay '<pipeline>'\n` +
    `  tokay run --mode tool '<pipeline>'\n` +
    `  tokay run path/to/workflow.tokay\n` +
    `  tokay run --file path/to/workflow.tokay --args-json '{...}'\n` +
    `  tokay run --dry-run --file path/to/workflow.tokay\n` +
    `  tokay run --dry-run '<pipeline>'\n` +
    `  tokay resume --token <token> --approve yes|no\n` +
    `  tokay resume --token <token> --response-json '{...}'\n` +
    `  tokay resume --token <token> --cancel\n` +
    `  tokay doctor\n` +
    `  tokay version\n` +
    `  tokay help <command>\n\n` +
    `Flags:\n` +
    ...
    `Examples:\n` +
    `  tokay 'exec --json "echo [1,2,3]" | json'\n` +
    `  tokay run --mode tool 'exec --json "echo [1]" | approve --prompt "ok?"'\n\n` +
    `Commands:\n` +
    `  exec, head, json, pick, table, where, approve, ask, hnsai.invoke, llm.invoke, llm_task.invoke, state.get, state.set, diff.last, commands.list, workflows.list, workflows.run\n`;
}
