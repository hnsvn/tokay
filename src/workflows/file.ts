...
function mergeEnv(
  ...
  // Example: $TOKAY_ARG_TEXT
  env.TOKAY_ARGS_JSON = JSON.stringify(args ?? {});
  for (const [key, value] of Object.entries(args ?? {})) {
    const normalized = normalizeArgEnvKey(key);
    if (!normalized) continue;
    env[`TOKAY_ARG_${normalized}`] = String(value);
  }
...
function parseApprovalTimeoutMs(env: Record<string, string | undefined>) {
  const raw = env?.TOKAY_APPROVAL_INPUT_TIMEOUT_MS;
...
function resolveToolEnvelopeMaxBytes(env: Record<string, string | undefined>) {
  const raw = env?.TOKAY_MAX_TOOL_ENVELOPE_BYTES;
