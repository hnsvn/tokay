...
export const workflowsRunCommand = {
  name: 'workflows.run',
  meta: {
    description: 'Run a named Tokay workflow',
    argsSchema: {
      type: 'object',
      properties: {
  ...
  help() {
    return `workflows.run — run a named Tokay workflow\n\nUsage:\n  workflows.run --name <workflow> [--args-json '{...}']\n\nExample:\n  workflows.run --name github.pr.monitor.notify --args-json '{"repo":"hnsvn/hnsai","pr":1152}'\n`;
  },
