...
export const stateGetCommand = {
  name: 'state.get',
  meta: {
    description: 'Read a JSON value from Tokay state',
    argsSchema: {
      type: 'object',
      properties: {
  ...
  help() {
    return `state.get — read a JSON value from Tokay state\n\nUsage:\n  state.get <key>\n\nEnv:\n  TOKAY_STATE_DIR overrides storage directory\n`;
  },
  async run({ args, ctx }) {
    const key = args._[0];
...
export const stateSetCommand = {
  name: 'state.set',
  meta: {
    description: 'Write a JSON value to Tokay state',
    argsSchema: {
      type: 'object',
      properties: {
        _: { type: 'array', items: { type: 'string' }, description: 'Key' },
