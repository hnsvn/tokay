...
export const emailTriageCommand = {
  ...
  meta: {
    ...
    argsSchema: {
      ...
      properties: {
        ...
        url: { type: 'string', description: 'Reserved for compatibility (ignored in HnsAi mode)' },
  ...
  async run({ input, args, ctx }) {
    ...
    const hasLlmProvider = Boolean(
      String(env.TOKAY_LLM_PROVIDER ?? '').trim() ||
      String(env.TOKAY_PI_LLM_ADAPTER_URL ?? '').trim() ||
      String(env.TOKAY_LLM_ADAPTER_URL ?? '').trim() ||
      String(env.HNSAI_URL ?? env.HUYD_URL ?? '').trim(),
    );
    ...
    const llmRes = await llmCmd.run({
      ...
      args: {
        ...
        'state-key': args['state-key'] ?? env.TOKAY_RUN_STATE_KEY,
      },
      ctx,
    } as any);
