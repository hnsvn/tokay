...
import type { TokayCommand } from '../types.js';
...
type BuiltInProvider = 'hnsai' | 'pi' | 'http';
...
export const llmInvokeCommand = createLlmInvokeCommand({
  ...
  helpConfig: [
    'Provider resolution order: --provider, TOKAY_LLM_PROVIDER, then environment auto-detect.',
    'Built-in providers: hnsai, pi, http.',
    'HnsAi provider uses HNSAI_URL (HUYD_URL also supported) and HNSAI_TOKEN.',
    'Pi provider uses TOKAY_PI_LLM_ADAPTER_URL and is intended to be supplied by a Pi extension.',
    'Generic http provider uses TOKAY_LLM_ADAPTER_URL and optional TOKAY_LLM_ADAPTER_TOKEN.',
  ],
  helpExamples: [
    "llm.invoke --prompt 'Write summary'",
    "llm.invoke --provider hnsai --model claude-3-sonnet --prompt 'Write summary'",
...
export const llmTaskInvokeCommand = createLlmInvokeCommand({
  ...
  defaultProvider: 'hnsai',
  description: 'Backward-compatible alias for llm.invoke using the HnsAi adapter',
  helpTitle: 'llm_task.invoke — backward-compatible alias for llm.invoke using HnsAi',
  helpConfig: [
    'Requires HNSAI_URL (or HUYD_URL) and optionally HNSAI_TOKEN.',
    'Use llm.invoke for new workflows and non-HnsAi adapters.',
  ],
  helpExamples: [
...
export function createLlmInvokeCommand(config: CommandConfig): TokayCommand {
  return {
    name: config.name,
    meta: {
      ...
      argsSchema: {
        ...
        properties: {
          provider: {
            type: 'string',
            description: 'LLM adapter provider (hnsai, pi, http). Optional if auto-detected.',
          },
          token: {
            type: 'string',
          ...
          'state-key': { type: 'string', description: 'Run-state key override (else TOKAY_RUN_STATE_KEY)' },
...
async function runLlmInvoke({ input, args, ctx, config }: { input: AsyncIterable<any>; args: any; ctx: any; config: CommandConfig }) {
  ...
  const schemaVersion = resolveEnvString(
    args['schema-version'],
    ['TOKAY_LLM_SCHEMA_VERSION', ...(config.legacyEnvCompat ? ['LLM_TASK_SCHEMA_VERSION'] : [])],
    env,
    'v1',
  );
  ...
  const maxValidationRetriesRaw =
    ...
    getFirstEnv(env, ['TOKAY_LLM_VALIDATION_RETRIES', ...(config.legacyEnvCompat ? ['LLM_TASK_VALIDATION_RETRIES'] : [])]);
  ...
  const forceRefresh = flag(
    args.refresh ??
      getFirstEnv(env, ['TOKAY_LLM_FORCE_REFRESH', ...(config.legacyEnvCompat ? ['LLM_TASK_FORCE_REFRESH'] : [])]),
  );
  const stateKey = String(args['state-key'] ?? env.TOKAY_RUN_STATE_KEY ?? '').trim() || null;
...
function resolveProvider(args: any, env: any, defaultProvider?: SupportedProvider | null, ctx?: any): SupportedProvider {
  const explicit = String(args.provider ?? env.TOKAY_LLM_PROVIDER ?? '').trim().toLowerCase();
  if (explicit) {
    if (explicit === 'hnsai' || explicit === 'pi' || explicit === 'http') {
      return explicit;
    }
  ...
  if (String(env.TOKAY_PI_LLM_ADAPTER_URL ?? '').trim()) return 'pi';
  if (String(env.HNSAI_URL ?? env.HUYD_URL ?? '').trim()) return 'hnsai';
  if (String(env.TOKAY_LLM_ADAPTER_URL ?? '').trim()) return 'http';
  throw new Error('llm.invoke could not resolve a provider. Set --provider or TOKAY_LLM_PROVIDER');
}
...
function resolveAdapter({
  ...
  if (provider === 'hnsai') {
    const hnsaiUrl = String(env.HNSAI_URL ?? env.HUYD_URL ?? '').trim();
    if (!hnsaiUrl) {
      throw new Error(`${config.name} requires HNSAI_URL (or HUYD_URL) for provider=hnsai`);
    }
    const endpoint = new URL('/tools/invoke', hnsaiUrl);
    const token = String(args.token ?? env.HNSAI_TOKEN ?? env.HUYD_TOKEN ?? '').trim();
    return {
      provider,
      source: config.sourceForProvider?.(provider) ?? 'hnsai',
      async invoke({ payload }) {
        return invokeHnsAiAdapter({ endpoint, token, payload });
      },
    };
  }

  if (provider === 'pi') {
    const adapterUrl = String(env.TOKAY_PI_LLM_ADAPTER_URL ?? '').trim();
    if (!adapterUrl) {
      throw new Error(`${config.name} requires TOKAY_PI_LLM_ADAPTER_URL for provider=pi`);
    }
    const token = String(args.token ?? env.TOKAY_PI_LLM_ADAPTER_TOKEN ?? '').trim();
    return {
      provider,
  ...
  const adapterUrl = String(env.TOKAY_LLM_ADAPTER_URL ?? '').trim();
  if (!adapterUrl) {
    throw new Error(`${config.name} requires TOKAY_LLM_ADAPTER_URL for provider=http`);
  }
  const token = String(args.token ?? env.TOKAY_LLM_ADAPTER_TOKEN ?? '').trim();
  return {
    provider,
...
async function invokeHnsAiAdapter({ endpoint, token, payload }: { endpoint: URL; token: string; payload: any }) {
  ...
  if (parsed && typeof parsed === 'object' && !Array.isArray(parsed) && 'ok' in parsed) {
    if (parsed.ok !== true) {
      ...
      throw new Error(`hnsai adapter error: ${msg}`);
    }
...
function resolveModel(args: any, env: any, legacyEnvCompat: boolean | undefined) {
  return resolveEnvString(
    args.model,
    ['TOKAY_LLM_MODEL', ...(legacyEnvCompat ? ['LLM_TASK_MODEL'] : [])],
    env,
    '',
  );
}
...
function normalizeResult({
  ...
  const item: NormalizedInvocationItem = {
    ...
    source,
    cached: source !== 'remote' && source !== 'hnsai' && source !== 'huyd' && source !== 'pi' && source !== 'http',
...
function getCacheDir(env: any) {
  if (env?.TOKAY_CACHE_DIR) return String(env.TOKAY_CACHE_DIR);
  return path.join(process.cwd(), '.tokay-cache');
}
