function createInvokeCommand(commandName: string) {
  return {
    name: commandName,
    meta: {
      description: 'Call a local HnsAi tool endpoint',
      argsSchema: {
        type: 'object',
        properties: {
          url: {
            type: 'string',
            description: 'HnsAi control URL (or HNSAI_URL / HUYD_URL)',
          },
          token: { type: 'string', description: 'Bearer token (or HNSAI_TOKEN / HUYD_TOKEN)' },
      ...
      sideEffects: ['calls_huyd_tool'],
    },
    help() {
      return (
        `${commandName} — call a local HnsAi tool endpoint\n\n` +
        `Usage:\n` +
        ...
        `Config:\n` +
        `  - Uses HNSAI_URL env var by default (or pass --url).\n` +
        `  - Backward compatible: HUYD_URL is also supported.\n` +
        `  - Optional Bearer token via HNSAI_TOKEN env var (or pass --token).\n` +
        `  - Backward compatible: HUYD_TOKEN is also supported.\n` +
        `  - Optional attribution via --session-key <sessionKey>.\n\n` +
        `Notes:\n` +
        `  - This is a thin transport bridge. Tokay should not own OAuth/secrets.\n`
      );
    },
    async run({ input, args, ctx }) {
      ...
      const url = String(args.url ?? ctx.env.HNSAI_URL ?? ctx.env.HUYD_URL ?? '').trim();
      if (!url) throw new Error(`${commandName} requires --url or HNSAI_URL`);

      const tool = args.tool;
      const action = args.action;
      if (!tool || !action) throw new Error(`${commandName} requires --tool and --action`);

      const token = String(args.token ?? ctx.env.HNSAI_TOKEN ?? ctx.env.HUYD_TOKEN ?? '').trim();
      ...
      if (!each) {
        // Drain input: for now we don't stream input into huyd calls.
...
export const hnsaiInvokeCommand = createInvokeCommand('hnsai.invoke');
export const huydInvokeCommand = createInvokeCommand('huyd.invoke');
