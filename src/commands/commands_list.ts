import type { CommandMeta, TokayCommand } from './types.js';
...
export const commandsListCommand: TokayCommand = {
  name: 'commands.list',
  help() {
    return (
      `commands.list — list available Tokay pipeline commands\n\n` +
      `Usage:\n` +
      `  commands.list\n\n` +
      `Notes:\n` +
      `  - Intended for agents (e.g. HnsAi) to discover available pipeline stages dynamically.\n` +
      `  - Output includes name/description plus optional metadata (argsSchema/examples/sideEffects) when provided by commands.\n`
    );
  },
  meta: {
    description: 'List available Tokay pipeline commands',
  ...
  async run({ input, ctx }) {
    ...
    const output = names.map((name) => {
      const cmd = ctx.registry.get(name) as TokayCommand | undefined;
