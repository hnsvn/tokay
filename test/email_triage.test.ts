...
test("email.triage --llm uses llm_task.invoke to draft replies (and can emit drafts)", async () => {
  ...
  const cacheDir = await mkdtemp(join(tmpdir(), "tokay-cache-"));
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      // HnsAi tool router envelope -> llm-task tool envelope
      res.end(
        JSON.stringify({
          ok: true,
          result: {
...
test("email.triage --llm honors HNSAI_URL (not just HUYD_URL)", async () => {
  const registry = createDefaultRegistry();
  const cacheDir = await mkdtemp(join(tmpdir(), "tokay-cache-"));
