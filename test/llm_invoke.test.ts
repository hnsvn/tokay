...
test('llm.invoke auto-detects HnsAi provider and normalizes output', async () => {
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx({ HNSAI_URL: `http://localhost:${port}`, TOKAY_CACHE_DIR: cacheDir }, registry),
    } as any);

    const items = await collect(result.output!);
    assert.equal(items.length, 1);
    assert.equal(items[0].kind, 'llm.invoke');
    assert.equal(items[0].source, 'hnsai');
...
test('llm.invoke uses Pi adapter over local HTTP bridge', async () => {
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx(
        {
          TOKAY_PI_LLM_ADAPTER_URL: `http://127.0.0.1:${port}`,
          TOKAY_LLM_MODEL: 'anthropic/claude-sonnet-4-5',
