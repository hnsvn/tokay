...
test('runToolRequest executes pipeline with injected llm adapter', async () => {
  ...
  const envelope = await runToolRequest({
    ...
    ctx: {
      env: {
        ...process.env,
        TOKAY_LLM_PROVIDER: 'pi',
        TOKAY_LLM_MODEL: 'test/model',
      },
      llmAdapters: {
        pi: adapter,
      },
...
test('resumeToolRequest completes approval-gated workflow with injected llm adapter', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-core-tool-runtime-'));
  ...
  const env = {
    ...process.env,
    TOKAY_STATE_DIR: path.join(tmpDir, 'state'),
    TOKAY_LLM_PROVIDER: 'pi',
    TOKAY_LLM_MODEL: 'test/model',
  };
...
test('runToolRequest/resumeToolRequest handles needs_input workflow pauses', async () => {
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-core-tool-input-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
