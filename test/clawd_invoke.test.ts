...
test('hnsai.invoke posts to /tools/invoke and returns JSON', async () => {
  ...
  try {
    ...
    const cmd = registry.get('hnsai.invoke');
...
test('hnsai.invoke --each maps input items into tool args', async () => {
