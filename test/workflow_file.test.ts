...
test('workflow file runs with approval and resume', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');

  const env = { ...process.env, TOKAY_STATE_DIR: stateDir };
...
test('workflow resume cancellation cleans up resume state', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-cancel-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow file input steps pause and resume with structured responses', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-input-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow input resumes preserve the full subject even when the tool envelope preview is truncated', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-input-truncate-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
  ...
  const env = {
    ...process.env,
    TOKAY_STATE_DIR: stateDir,
    TOKAY_MAX_TOOL_ENVELOPE_BYTES: '8192',
  };
...
test('workflow approval resumes require an explicit decision', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-approval-required-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow conditions support comparisons, boolean operators, and parentheses', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-conditions-'));
  const stateDir = path.join(tmpDir, 'state');
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow conditions reject standalone bare identifiers', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-condition-invalid-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow conditions reject unknown step refs even under negation', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-condition-typo-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow conditions compare object refs without key-order sensitivity', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-condition-object-eq-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('workflow files can mix shell steps, approval-only steps, and pipeline llm steps', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-mixed-'));
  const stateDir = path.join(tmpDir, 'state');
  ...
  const env = {
    ...process.env,
    TOKAY_STATE_DIR: stateDir,
    TOKAY_CACHE_DIR: cacheDir,
    TOKAY_LLM_ADAPTER_URL: `http://127.0.0.1:${port}`,
  };
...
test('workflow pipeline steps respect cwd and feed later shell steps via stdout refs', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-workflow-pipeline-cwd-'));
  ...
  const result = await runWorkflowFile({
    ...
    ctx: {
      ...
      stdout: process.stdout,
      stderr: process.stderr,
      env: { ...process.env, TOKAY_STATE_DIR: path.join(tmpDir, 'state') },
      mode: 'tool',
      registry,
    },
  });
