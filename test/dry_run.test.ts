...
function runTokay(args: string[], opts?: { env?: Record<string, string | undefined> }) {
  const res = spawnSync(process.execPath, [path.join('bin', 'tokay.js'), ...args], {
    cwd: path.resolve('.'),
...
test('dry-run of a 3-step workflow file (shell steps)', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
...
test('dry-run of a workflow with an approval step', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-approval-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run of a workflow with a conditional step that would be skipped', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-cond-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run of an inline pipeline string via CLI', async () => {
  const res = runTokay(['run', '--dry-run', 'exec --json "echo [1,2,3]" | where active=true | table']);
...
test('dry-run exits 0 on valid input', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-exit-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');

  const res = runTokay(['run', '--dry-run', '--file', filePath]);
...
test('normal run still works (no regression)', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-regression-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');

  const res = runTokay(['run', '--file', filePath]);
...
test('dry-run approval step sets approved:true so downstream conditions evaluate correctly', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-approvcond-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run workflow with pipeline step', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-pipeline-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run throws on stdin ref to non-existent step', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-stdin-bad-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run annotates valid stdin step refs as unknown at plan time', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-stdin-valid-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run preserves step output refs in shell commands instead of collapsing them to empty strings', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-shellrefs-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run still resolves approved refs in shell commands', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-approvedref-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run throws on pipeline step with unknown command', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-badcmd-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run flag is consumed regardless of position in argv', async () => {
  ...
  const before = runTokay(['run', '--dry-run', 'exec --json "echo [1]"']);
  assert.equal(before.status, 0);
  assert.match(before.stderr, /\[DRY RUN\]/);
  assert.equal(before.stdout.trim(), '');

  // --dry-run after positional file arg: tokay run workflow.tokay --dry-run
...
test('dry-run flag after positional file arg activates dry-run', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-test-'));
  const filePath = path.join(tmpDir, 'test.tokay');
  ...
  const res = runTokay(['run', filePath, '--dry-run']);
...
test('dry-run flag still activates after positional workflow file when other Tokay flags are present', async () => {
  const workflow = {
    args: { name: { default: 'hello' } },
    steps: [{ id: 'greet', run: 'echo ${name}' }],
  };
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-test-args-'));
  const filePath = path.join(tmpDir, 'test.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow), 'utf8');

  const res = runTokay(['run', filePath, '--args-json', '{"name":"world"}', '--dry-run']);
  assert.equal(res.status, 0, `expected exit 0\nstdout=${res.stdout}\nstderr=${res.stderr}`);
  assert.match(res.stderr, /\[DRY RUN\]/);
...
test('command-level --dry-run in the first stage is not stolen by Tokay dry-run parsing', () => {
  const res = runTokay(['run', 'hnsai.invoke', '--tool', 'message', '--action', 'send', '--dry-run']);

  assert.notEqual(res.status, 0, 'expected command execution to fail instead of Tokay dry-run exiting 0');
  assert.doesNotMatch(res.stderr, /\[DRY RUN\]/);
  assert.match(res.stderr, /requires --url or HNSAI_URL/);
});
...
test('dry-run allows pipeline stage names that still depend on prior step output', async () => {
  ...
  const workflow = {
    steps: [
      { id: 'planner', run: 'echo hnsai.invoke' },
      { id: 'call', pipeline: '$planner.stdout --tool message --action send' },
    ],
  };

  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-dynamic-stage-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
...
test('dry-run does not eat shell variables that only resemble step refs', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-shell-vars-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');

  const res = runTokay(['run', '--dry-run', filePath]);
...
test('dry-run evaluates compound conditions with approvals, input placeholders, and parentheses', async () => {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'tokay-dry-compound-cond-'));
  const filePath = path.join(tmpDir, 'workflow.tokay');
  await fsp.writeFile(filePath, JSON.stringify(workflow, null, 2), 'utf8');
