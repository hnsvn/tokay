...
test('llm_task.invoke posts to /tools/invoke (huyd) and normalizes result', async () => {
  const registry = createDefaultRegistry();
  const cmd = registry.get('llm_task.invoke');
  assert.ok(cmd, 'llm_task.invoke should be registered');
  const cacheDir = await mkdtemp(path.join(tmpdir(), 'tokay-cache-'));
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx({ TOKAY_CACHE_DIR: cacheDir, HUYD_URL: `http://localhost:${port}` }, registry),
    } as any);
    ...
    assert.equal(payload.source, 'huyd');
...
test('llm_task.invoke persists to run state so resume skips remote call', async () => {
  const stateDir = await mkdtemp(path.join(tmpdir(), 'tokay-state-'));
  const registry = createDefaultRegistry();
  const cmd = registry.get('llm_task.invoke');
  assert.ok(cmd);
  ...
  const cacheDir = await mkdtemp(path.join(tmpdir(), 'tokay-cache-'));
  const ctxEnv = { TOKAY_STATE_DIR: stateDir, TOKAY_CACHE_DIR: cacheDir };
  ...
  try {
    const first = await cmd.run({
      ...
      ctx: baseCtx({ ...ctxEnv, HUYD_URL: `http://localhost:${port}` }, registry),
    } as any);
    const firstItems = await collect(first.output!);
    assert.equal(firstItems[0].source, 'huyd');
...
test('llm_task.invoke reuses file cache when URL unavailable', async () => {
  ...
  const ctxEnv = { TOKAY_CACHE_DIR: cacheDir, HUYD_URL: `http://localhost:${port}` };
...
test('llm_task.invoke uses HUYD_URL (/tools/invoke) without requiring --url/--model', async () => {
  const registry = createDefaultRegistry();
  const cmd = registry.get('llm_task.invoke');
  assert.ok(cmd);
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      // This is the HnsAi tool router envelope.
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(
        JSON.stringify({
          ok: true,
  ...
  try {
    const result = await cmd.run({
      ...
      ctx: baseCtx({ HUYD_URL: `http://localhost:${port}`, TOKAY_CACHE_DIR: cacheDir }, registry),
    } as any);

    const items = await collect(result.output!);
    assert.equal(items.length, 1);
    assert.equal(items[0].source, 'huyd');
    assert.equal(items[0].cached, false);
    assert.equal(items[0].runId, 'task_huyd_1');
