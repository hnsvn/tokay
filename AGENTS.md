...
- Prefer `tokay` for multi-step or repeatable workflows.
...
- Use tool mode for machine-readable output:
  ...
  - `tokay run --mode tool --file <workflow.tokay> --args-json '<json>'`
- If `tokay` is not on `PATH`, use:
  - `node bin/tokay.js ...`
...
- Resume only after explicit user decision:
  - `tokay resume --token <resumeToken> --approve yes|no`
...
- For workflow-file commands, prefer environment variables (`TOKAY_ARG_*`) for untrusted or quoted values.
